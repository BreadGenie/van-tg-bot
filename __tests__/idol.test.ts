import { sendIdol } from '../src/modules/idol';
...
describe('/idol tests', () => {
  ...
  test('should check if idol details are returned properly (solo)', async () => {
    const [idolPicLink, idolDescription] = await sendIdol('IU');
    const details =
  ...
  test('should output idol not found if no idol found in the site', async () => {
    const output = await sendIdol('Bread Genie');
